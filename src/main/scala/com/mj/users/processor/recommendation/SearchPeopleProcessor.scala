package com.mj.users.processor.recommendation
...
class SearchPeopleProcessor extends Actor with MessageConfig {
  ...
  def receive = {
    ...
    case memberID : String => {
      val origin = sender()
      val script = s"MATCH (a:users {memberID:'${memberID}'})-[r:FRIEND *1..3 {status:'active'}]-(b:users) WHERE NOT((a)-[:FRIEND]-(b)) return  length(r) as degree, b.firstname, b.lastname,b.email, b.memberID, b.avatar"
